import { AllInputs, Checks, Select, Options } from "../../schemas/inputs";
	...
	return (
		<section className="d-flex justify-content-center p-3">
			...
			>
				...
				{AllInputs.map(({ type, id, label, key, required, message }) => (
				...
				>
					{Checks.map(({ type, key, id, value, label, required }) => (
						<Label check key={key}>
							<Input
								type={type}
								id={id}
				...
				<FormGroup className="pt-2">
					...
					<Row form>
						{Select.map(({ id, type, key, options, required }) => (
							<Col key={id}>
